    ...
    <div className="max-w-[1600px] mx-auto flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          {allocations.map((a) => (
            <div key={a.channel} className="flex items-center gap-3 md:gap-4">
              <span className="text-xs text-muted-foreground w-28 md:w-40 truncate shrink-0">{a.channel}</span>
              <Progress value={(a.spent / a.budget) * 100} className="flex-1 h-2.5" />
              <span className="text-xs text-muted-foreground w-24 md:w-28 text-right shrink-0">{fmt(a.spent)} / {fmt(a.budget)}</span>
            </div>
          ))}
        </CardContent>
      </Card>
            ...
            <TableBody>
              {transactions.map((t) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell className="text-xs max-w-[150px] truncate">{t.desc}</TableCell>
                  <TableCell className="text-xs font-semibold">{t.amount}</TableCell>
                  <TableCell className="text-xs text-muted-foreground hidden sm:table-cell">{t.date}</TableCell>
                  <TableCell>
